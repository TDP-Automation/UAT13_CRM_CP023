...
'			JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Pago (Orden 599306A").JavaCheckBox("Financiamiento Externo").Set "ON"
	...
	Wend
	Dim z
	z = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaEdit("BAR ID").GetROProperty("text")
	While z = ""
		wait 1
		z = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaEdit("BAR ID").GetROProperty("text")
	Wend
...
Sub TipoEnvio()
	...
	Select Case ucase(str_metodo_entrega)
		...
		Case "DELIVERY"
				...
				wait 1
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar dirección de_2").JavaEdit("TextFieldNative$1").Set "45141994"
				wait 1
...
Sub EnvioPago()
	JavaWindow("Ejecutivo de interacción").JavaMenu("Buscar").JavaMenu("Pedidos").Select
		...
		imagenToWord "Enviar Pago",RutaEvidencias() & "EnvioPago.png"
		wait 1
...
'					If vardisp<>str_idDispositivo Then
						...
						ElseIf str_MotivoCambio="CAEQ_SIM" Then
							...
							Set shell = CreateObject("Wscript.Shell") 
							shell.SendKeys "{ENTER}"
							JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaTable("SearchJTable").SetCellData "#1","#4",DataTable("e_SerieSIM", dtLocalSheet)
							wait 1
						ElseIf str_MotivoCambio="CAEQ_EQUIPO" Then
							JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaTable("SearchJTable").DoubleClickCell "#1","#4"
							Set shell = CreateObject("Wscript.Shell") 
